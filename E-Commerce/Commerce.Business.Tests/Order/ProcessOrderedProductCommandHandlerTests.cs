...
public sealed class ProcessOrderedProductCommandHandlerTests
{
    ...
    public ProcessOrderedProductCommandHandlerTests()
    {
        orderedProduct = OrderedProductsFactory.WithIds(Guid.NewGuid(), Guid.NewGuid(), identifiedUser.Id).Confirmed();
    ...
    public async Task When_OrderNotFound_Then_ShouldFail()
    {
        ...
        result.Error.Should().Be(BusinessErrors.OrderedProduct.Process.OrderNotFound);
    ...
    public async Task When_OrderedProductNotFound_Then_ShouldFail()
    {
        ...
        // Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Should().Be(BusinessErrors.OrderedProduct.Process.OrderedProductNotFound);
    ...
    public async Task When_CallerIsNotSeller_Then_ShouldFail()
    {
        ...
        // Assert
        result.IsFailure.Should().BeTrue();
        result.Error.Should().Be(BusinessErrors.OrderedProduct.Process.OrderedProductNotSoldByCaller);
    ...
    private ProcessOrderedProductCommand Command() => new(order.Id, orderedProduct.Id);

    private ProcessOrderedProductCommandHandler Sut() => new(identifiedUser, orderRepository, orderedProductRepository);
}